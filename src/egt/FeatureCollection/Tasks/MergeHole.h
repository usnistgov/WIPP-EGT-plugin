//
// Created by gerardin on 9/24/19.
//

#ifndef NEWEGT_MERGEHOLE_H
#define NEWEGT_MERGEHOLE_H
#include <tiffio.h>
...
namespace egt {
    ...
    class MergeHole : public htgs::ITask<ViewAnalyseBlock, ViewAnalyse> {

    public:

        explicit MergeHole(size_t numThreads, pb::Pyramid &pyramid) : ITask(numThreads) , pyramid(pyramid) {}
...
//                addToNextLevelMerge(view, {row - 1, col - 1}, {data->getRow() - 1, data->getCol() - 1}); //TOP-LEFT
        ...
        ITask<ViewAnalyseBlock, ViewAnalyse> *copy() override {
            return new MergeHole(this->getNumThreads(), this->pyramid);
        }
