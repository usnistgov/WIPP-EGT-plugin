...
/**
  ...
  **/
    ...
    EGTGradientViewAnalyzer(size_t numThreads,
            ...
            : ITask<GradientView < UserType>, ViewOrViewAnalyse<UserType>>
    ...
    void executeTask(std::shared_ptr<GradientView < UserType>>
    ...
    false); //clear container that keeps track of all visited pixels in a pass through the image.
...
EGTGradientViewAnalyzer <UserType> *copy()
...
void flattenPixelToMerge(Blob *blob, Color blobColor) {
    ...
    //iterate the merge structure
    while (mergeIterator != endIterator) {
        auto blobMapIt = mergeIterator->second.find(
                blob);
